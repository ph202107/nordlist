...
#
# =====================================================================
# The Main Menu starts on line 1481.  Recommend configuring the
...
function change_setting {
    ...
    # $4 indicator                  $ac
    # $5 location                   $acwhere
    # $6 override                   disable fast2, main_menu
    #
    echo -e "$4 $1 is ${LColor}$2${Color_Off}."
    echo
    if [[ "$fast2" =~ ^[Yy]$ ]] && [[ "$6" != "override" ]]; then
        echo
...
function fobfuscate {
    ...
    if [[ "$technology" == "NordLynx" ]]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            echo
            echo -e "Keep Obfuscate ${LColor}$obfuscate${Color_Off}."
            echo
        fi
    fi
    main_menu
