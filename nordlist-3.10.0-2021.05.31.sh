...
#       "figlet" (ASCII generator) and "lolcat" (colorizer).
...
# (location must support obfuscation)  eg obwhere="United_States"
obwhere=""
#
# Specify your Auto-Connect location.  eg acwhere="Canada Vancouver"
acwhere=""
#
# Specify your default alternate DNS servers with description
default_dns="103.86.96.100 103.86.99.100"; dnsdesc="Nord"
#default_dns="94.140.14.14 94.140.15.15"; dnsdesc="AdGuard"
#default_dns="9.9.9.9 149.112.112.112"; dnsdesc="Quad9"
#
...
alwaysrate="n"
#
# Prompt before connecting to a city when there's only one choice.
# "y" to prompt first. "n" to connect without prompting.
cityprompt="y"
#
...
#
#
# The Main Menu starts on line 1283.
...
function logo {
    ...
    echo -e "$transfer   $ip"
    echo -e $uptime $ks$cs$ob
    echo
    # all indicators: $fw$ks$cs$ob$no$ac$ip6$dns
}
function heading () {
    clear
...
function cities {
    ...
    citylist+=( "Exit" )
    numcities=${#citylist[@]}
    if [[ "$numcities" == "3" ]] && [[ "$cityprompt" =~ ^[Nn]$ ]]; then
...
function fipversion6 {
    heading "IP version 6"
    echo "Enable or disable NordVPN IPv6 support."
    echo
    echo -e "$ip6 IPv6 is ${LColor}$ipversion6${Color_Off}."
    echo
    read -n 1 -r -p "Change the IPv6 setting? (y/n) "
    echo
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        if [[ "$ipversion6" == "enabled" ]]; then
            nordvpn set ipv6 disabled; wait
        else
            nordvpn set ipv6 enabled; wait
        fi
    else
        echo "No changes made."
    fi
...
function fcustomdns {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        if [[ "$dns_set" == "disabled" ]]; then
            ...
            echo "Default ($dnsdesc): $default_dns"
            echo
...
function fdefaults {
    echo
    echo "Disconnect and set variables to the default"
    echo "config specified in 'function set_defaults'."
    echo
    read -n 1 -r -p "Proceed? (y/n) "
    echo
    echo
...
function set_defaults {
    ...
    if [[ "$autocon" == "disabled" ]]; then     # location
        nordvpn set autoconnect enabled Canada Vancouver
    fi
    # ipv6 dns and whitelist commands
    echo
...
do
    case $opt in
        ...
        "Settings")
            ...
            heading "Settings"
            echo
            echo -e "($technology $p_info) $fw$ks$cs$ob$no$ac$ip6$dns"
            echo
            PS3=$'\n''Choose a Setting: '
            submsett=("Technology" "Protocol" "Firewall" "KillSwitch" "CyberSec" "Obfuscate" "Notify" "AutoConnect" "IPv6" "Custom-DNS" "Whitelist" "Account" "Restart" "Reset" "Tools" "Defaults" "Exit")
