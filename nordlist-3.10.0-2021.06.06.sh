...
#
# =====================================================================
# The Main Menu starts on line 1369.  Recommend configuring the
...
function change_setting {
    ...
    if [[ "$fast2" =~ ^[Yy]$ ]]; then
        ...
        read -n 1 -r -p "Change the $1 setting? (y/n) "
        echo
        echo
    fi
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
function ffirewall {
    ...
    echo
    change_setting "Firewall" "$firewall" "firewall" "$fw"
}
function fkillswitch {
    ...
    echo
    change_setting "Kill Switch" "$killswitch" "killswitch" "$ks"
}
function fcybersec {
    heading "CyberSec"
    echo
...
function fautoconnect {
    ...
    change_setting "Auto-Connect" "$autocon" "autoconnect" "$ac" "$acwhere"
}
...
function ftools {
    ...
    nettools=("Rate VPN Server" "www.speedtest.net" "youtube-dl" "ping vpn" "ping google" "my traceroute" "ipleak.net" "world map" "Exit")
