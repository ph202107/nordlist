...
#
# =====================================================================
# The Main Menu starts on line 1652.  Recommend configuring the
...
function fobservers {
    ...
    echo
    echo -e "Current settings: ${LColor}$technology $pdisplay $fw$ks$ob${Color_Off}"
    echo
...
function ffirewall {
    ...
    if [[ "$firewall" == "enabled" ]] && [[ "$killswitch" == "enabled" ]]; then
        ...
        echo
        echo -e "${WColor}The Kill Switch must be disabled before"
        echo -e "disabling the Firewall.${Color_Off}"
        echo
        change_setting "the Kill Switch" "$killswitch" "killswitch" "$ks" "override"
        echo
        set_vars
        if [[ "$killswitch" == "enabled" ]]; then
            echo -e "$fw Keep the Firewall ${LColor}$firewall${Color_Off}."
            main_menu
        fi
    fi
...
function nordapi {
    ...
    echo
    echo -e "Current server: ${LColor}$nordhost${Color_Off}"
    echo
    PS3=$'\n''API Call: '
...
function ftools {
    ...
    heading "Tools"
    if [[ "$connected" == "Connected" ]]; then
        echo -e "Server: ${LColor}$nordhost${Color_Off} $ipaddr"
        echo "($technology $pdisplay) $city $country"
        ...
        echo
        read -r -p "Hostname/IP [Default $default_host]: " nordhost
        nordhost=${nordhost:-$default_host}
        echo
        echo -e "Use server: ${LColor}$nordhost${Color_Off}"
...
function fdefaults {
    echo
    echo "[Defaults] Disconnect and apply the NordVPN settings"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        set_defaults
        echo
        read -n 1 -r -p "[Defaults] Go to the 'Whitelist' setting? (y/n) "
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            fwhitelist back
        fi
